...
import { useAddCategaryMutation, useAddNewCategoryMutation, useGetCategoriesQuery, useGetIndustryQuery, useGetLanguagesQuery, usePostIndustryMutation } from '../../Components/all-products/allproductsApi/allProductsApi';
...
function IndustryCreate() {
    ...
    if (langData) {
      if (!params) {
        const maped = langData.map((item) => {
          return { name: "", language_id: item._id, parent_id: null, lable: item.name, order_level: "", type: "", banner: "", meta_title: "", meta_description: '', commision_rate: "", level: "", top: false, featured: false, meta_keyword: "", slug: "", description: '' }
        })
        setVal(maped)
      } else {
        const maped = langData.map((item) => {
  ...
  const getDetailCat = async () => {
    try {
      const res = await axios.get(`https://onlineparttimejobs.in/api/industry/admin/${params.id}`, {
        headers: {
          "Content-type": "application/json; charset=UTF-8",
          Authorization: `Bearer ${token}`,
        },
                ...
                <TabContext value={value}>
                  ...
                  {val && val.map((item, i) => {
                    return <TabPanel value={i}>
                      <div className="card">
                        <MultiLangIndustry setValue={setValue} data={val} item={item} i={i} addNewAttributeData={addNewAttributeData} onChangeHandler={onChangeHandler} onChangeHandlesr={onChangeHandlesr} />
                      </div>
