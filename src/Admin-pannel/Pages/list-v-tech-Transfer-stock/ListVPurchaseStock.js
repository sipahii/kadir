...
import VShowModal from "./VShowModal"

function ListVPurchaseStock() {
    const [data, setData] = useState(null)

    const getData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/serviceStockTransfer`)
        setData(res.data)
    }
    ...
    return <div className="aiz-main-content" style={{ width: "1020px", margin: "auto" }}>
        ...
        {show && <VShowModal show={show} handleClose={handleClose} id={valId} />}

    </div>
}
export default ListVPurchaseStock