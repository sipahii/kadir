...
function ReportsLedgers() {
    ...
    const SendValue = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/accountVoucher/reportBtLedgerId/${valState + '&' + dateFrom + '&' + dateTo}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
                        ...
                        <div className="col-4">
                            ...
                            <Form.Select aria-label="Default select example" value={valState} onChange={(e) => { setValState(e.target.value) }}>
                                ...
                                {underSelect && underSelect.map((item) => {
                                    return <option value={item._id}>{item?.AccLedgerName}</option>
                                })}
                            </Form.Select>
                        </div>
                        <div className="col-3">
