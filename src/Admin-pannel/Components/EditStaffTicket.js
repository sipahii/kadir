...
function EditStaffTicket() {
    ...
    const sendMessageData = async () => {
        ...
        fromData.append('description', messageVal.description);
        fromData.append('ticket_id', messageVal.ticket_id);
        fromData.append('image', messageImage);
        try {
            const res = await axios.post('https://onlineparttimejobs.in/api/ccm_ticketReply/add_Reply', fromData,
                {
                    headers: {
