...
function SeoMetaTagsAdmin() {
    ...
    const [seoMetaTagVal, setSeoMetatagVal] = useState({ meta_title: '', meta_description: '', meta_image: '' });

    const onChangeHandler = (e) => {
        const inpName = e.target.name;
        const inpVal = e.target.value;
        const clonedObj = { ...seoMetaTagVal };
        clonedObj[inpName] = inpVal;
        // setSeoMetatagVal(clonedObj)
    };

    const getData = () => {
        // console.log(seoMetaTagVal)
    }
