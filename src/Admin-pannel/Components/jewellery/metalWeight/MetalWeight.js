...
function MetalWeight() {
    ...
    const getdata = async () => {
        setLoading(true)
        try {
            const res = await axios.get('https://onlineparttimejobs.in/api/weightMaster', {
                headers: {
    ...
    const toastSuccessMessage = () => {
        toast.success("Metal Weight Deleted", {
            position: "top-center"
        })
    };
    const toastErrorMessage = () => {
        toast.error("Metal Weight Not Deleted ", {
            position: "top-center"
        })
    };
    ...
    const deletData = async (id) => {
        try {
            const res = await axios.delete(`https://onlineparttimejobs.in/api/weightMaster/delete_weight/${id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
                ...
                <div className="px-15px px-lg-25px">
                    ...
                    <div className="card">
                        <div className="card-header">
                            <h5 className="mb-0 h6">Metal Weight</h5>
                            <div className="col-md-6 text-md-right">
                                <Link to="/admin/add-metal-weight" className="btn btn-circle btn-info">
                                    <span>Add MetalWeight</span>
                                </Link>
                            </div>
                        </div>
