...
import { useCatagaryActiveMutation, useDeleteCategoryMutation, useDeleteIndexGroupMutation, useGetCategoriesQuery, useGetIndexGroupQuery } from "../all-products/allproductsApi/allProductsApi";
import { ToastContainer, toast } from "react-toastify";


function ListIndexGroupComp() {
    const [inputVal, setInputVal] = useState({ search: '' });
    const [blankArr, setBlankArr] = useState([]);

    const token = window.localStorage.getItem('token')
    // const { isLoading, data, refetch } = useGetCategoriesQuery(token);
    const { isLoading, data, refetch } = useGetIndexGroupQuery(token);
    ...
    const [deleteIndexGroup, res] = useDeleteIndexGroupMutation();

    function deleteCategoryData(id) {
        deleteIndexGroup({ id, token })
    };

    useEffect(() => {
        if (res.isSuccess == true) {
            alert('IndexGroup deleted successfully')
            refetch()
        }
    }, [res.isSuccess])
    ...
    const toastSuccessMessage = () => {
        toast.success("IndexGroup Updated Successfully", {
            position: "top-center"
        })
    };

    const toastErrorMessage = () => {
        toast.error("IndexGroup Update Faild..", {
            position: "top-center"
        })
    };
                ...
                <div className="px-15px px-lg-25px">
                    <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div className="row align-items-center">
                            <div className="col-md-6">
                                <h1 className="h3">All Index Group</h1>
                            </div>
                            <div className="col-md-6 text-md-right">
                                <Link to="/admin/indexGroup/create" className="btn btn-primary">
                                    <span>Add New Index Group</span>
                                </Link>
                            </div>
                    ...
                    <div className="card">
                        ...
                        <div className="card-header d-block d-md-flex">
                            <h5 className="mb-0 h6">Index Group</h5>
...
export default ListIndexGroupComp;