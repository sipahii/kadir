...
import AddLengthJewelMultiLang from "./AddLengthJewelMultiLang";

function AddLenngthJewelComp() {
    ...
    const toastSuccessMessage1 = () => {
        toast.success("Jwellery Length Updated", {
            position: "top-center"
        })
    };
    const toastErrorMessage1 = () => {
        toast.error("Jwellery Length Not Updated ", {
            position: "top-center"
        })
    };

    const toastSuccessMessage2 = () => {
        toast.success("Jwellery Length Added", {
            position: "top-center"
        })
    };
    const toastErrorMessage2 = () => {
        toast.error("Jwellery Length Not Added", {
            position: "top-center"
        })
    };

    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/length/${params?.uid}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const sendData = async () => {
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/length/update_length/${params?.uid}`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/length/add_length`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
                ...
                <TabContext value={value}>
                    ...
                    {val && val.map((item, i) => {
                        return <TabPanel value={i}>
                            <div className="card">
                                <AddLengthJewelMultiLang setValue={setValue} data={val} params={params} item={item} i={i} sendData={sendData} onChangeHandler={onChangeHandler} onChangeThumbnailImage={onChangeThumbnailImage} showImageD={showImageD} setShowImageD={setShowImageD} />
                            </div>
...
export default AddLenngthJewelComp
