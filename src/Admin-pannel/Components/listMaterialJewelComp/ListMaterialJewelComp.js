...
function ListMaterialJewelComp() {
    ...
    const getdata = async () => {
        setLoading(true)
        try {
            const res = await axios.get('https://onlineparttimejobs.in/api/material', {
                headers: {
    ...
    const toastErrorMessage = () => {
        toast.error("Material Not Deleted ", {
            position: "top-center"
        })
    };
    ...
    const deletData = async (id) => {
        setLoading(true)
        try {
            const res = await axios.delete(`https://onlineparttimejobs.in/api/material/delete_material/${id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
...
export default ListMaterialJewelComp