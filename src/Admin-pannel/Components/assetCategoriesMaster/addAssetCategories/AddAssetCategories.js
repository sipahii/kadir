...
function AddAssetCategories() {
    ...
    const selectSubCategory = async () => {
        const getSelectSubCategory = await axios.get(`${base_Url_Assets}assetSubCategories`)
    ...
    const postData = async () => {
        ...
        formData.append('pickupLocation', add.pickupLocation)

        console.log('BabukaformData---------------', formData)
    ...
    const getAssetId = async () => {
        const res = await axios.get(`${base_Url_Assets}asset/token`)
        const token = res.data
        console.log('token', token)
        const clonedData = { ...add }
        clonedData.id = token.id;
        setAdd(clonedData)
        // setAssetId(token)
    }
                                        ...
                                        <div className="card-body">
                                            ...
                                            <section className="form-section">
                                                <div className="row">
                                                    ...
                                                    {/* <div className="col-lg-3 mt-3">
                                                        <button type='button' onClick={handleClick} className="btn btn-primary mr-3">Save</button>
                                    ...
                                    <div className="card">
                                        ...
                                        <div className="card-body">
                                            ...
                                            <section className="form-section">
                                                ...
                                                <div className="col-lg-3 mt-3">
                                                    <button type='button'
                                                        disabled={!add.id.trim() || !add.modelNo.trim() || !add.serviceTag.trim() || !add.name.trim() || !add.description.trim() || !add.unit_price.trim() || !add.assetStatus.trim() || !add.dateOfPurchase.trim() || !add.category.trim() || !add.subCategory.trim() || !add.supplier.trim() || !add.department.trim() || !add.subDepartment.trim() || !add.dateOfManufacture.trim() || !add.yearOfValuation.trim() || !add.depriciation.trim() || !add.location.trim() || !add.note.trim() || !add.assignTo.trim()}
                                                        onClick={handleClick}
