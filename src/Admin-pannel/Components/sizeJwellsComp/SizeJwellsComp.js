...
function SizeJwellsComp() {
    ...
    const getdata = async () => {
        setLoading(true)
        try {
            const res = await axios.get('https://onlineparttimejobs.in/api/sizeJewel', {
                headers: {
    ...
    const toastSuccessMessage = () => {
        toast.success("Jewellery Size Deleted", {
            position: "top-center"
        })
    };
    const toastErrorMessage = () => {
        toast.error("Jewellery Size Not Deleted ", {
            position: "top-center"
        })
    };
    ...
    const deletData = async (id) => {
        try {
            const res = await axios.delete(`https://onlineparttimejobs.in/api/sizeJewel/delete_size/${id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
                    ...
                    {/* <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div className="row align-items-center">
                            <div className="col-md-6">
                                <h1 className="h3">Size</h1>
                            </div>
                        </div>
                    </div> */}
                    <div className="card">
                        <div className="card-header">
                            <h5 className="mb-0 h6">Size</h5>
                            <div className="col-md-6 text-md-right">
                                <Link to="/admin/add-size-jwells" className="btn btn-info">
                                    <span>+ Add Size</span>
                                </Link>
                            </div>
                        </div>
