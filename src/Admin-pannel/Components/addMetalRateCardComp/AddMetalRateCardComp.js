...
function AddMetalRateCardComp() {
    const [inputval, setInputVal] = useState({
        name: '', code: '', country_id: '', metalType_id: '', description: '', isActive: false
    })
    ...
    const toastErrorMessage1 = () => {
        toast.error("Metal Rate Not Updated ", {
            position: "top-center"
        })
    };
    const toastSuccessMessage2 = () => {
        toast.success("Metal Rate Added", {
            position: "top-center"
        })
    };
    const toastErrorMessage2 = () => {
        toast.error("Metal Rate Not Added", {
            position: "top-center"
        })
    };
    ...
    const sendData = async () => {
        if (params?.id) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/rateCard/metal/update_metalType/${params?.id}`, inputval, {
                    headers: {
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/rateCard/metal/add_metalType`, inputval, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
    ...
    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/rateCard/metal/${params.id}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
                ...
                <div className="px-15px px-lg-25px">
                    <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '3px' }}>
                            <div className="">
                                <h1 className="h3">{params.id ? 'Update' : 'Add'} Metal</h1>
                            </div>
                        </div>
                    </div>
                    <div className="card">
                        <div className="card-header">
                            <h5>{params.id ? 'Update' : 'Add'} Metal</h5>
                        </div>
...
export default AddMetalRateCardComp