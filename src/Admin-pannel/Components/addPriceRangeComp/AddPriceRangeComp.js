...
import AddpriceRangeMultiLang from './AddpriceRangeMultiLang';
import { useAddCelebrityPriceRangeMutation, useGetLanguagesQuery } from '../all-products/allproductsApi/allProductsApi';


function AddPriceRangeComp() {
    const token = window.localStorage.getItem('token')


    const toastSuccessMessage = () => {
        toast.success("PriceRange added Successfully", {
            position: "top-center"
        })
    };
    ...
    const submitEditPriceRangeData = async (data) => {
        const url = `https://onlineparttimejobs.in/api/influence_priceRange/${params.id}`
        try {
    ...
    const [addPricerange, res] = useAddCelebrityPriceRangeMutation();
    ...
    const addNewAttributeData = async (e) => {
        ...
        if (params?.id) {
            ...
            submitEditPriceRangeData(clone)
        } else {
            ...
            addPricerange({ data: { list: clone }, token: token })
        }
    };
    ...
    const getDetailCat = async () => {
        try {
            const res = await axios.get(`https://onlineparttimejobs.in/api/influence_priceRange/admin/${params.id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
                    ...
                    <div className="row">
                        <div className="col-lg-8 mx-auto">
                            {spcOr && <div className="preloaderCount">
                                ...
                                <h6>please wait your PriceRange uploading</h6>
                            </div>}
                                ...
                                <TabContext value={value}>
                                    ...
                                    {val && val.map((item, i) => {
                                        return <TabPanel value={i}>
                                            <div className="card">
                                                <AddpriceRangeMultiLang setValue={setValue} data={val} item={item} i={i} addNewAttributeData={addNewAttributeData} onChangeHandler={onChangeHandler} onChangeHandlesr={onChangeHandlesr} />
                                            </div>
...
export default AddPriceRangeComp