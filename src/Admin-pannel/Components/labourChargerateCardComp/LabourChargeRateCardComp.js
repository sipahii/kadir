...
function LabourChargeRateCardComp() {
    const [getListData, setgetListData] = useState();
    const [loading, setLoading] = useState(true)
    const token = window.localStorage.getItem('adminToken')

    const getdata = async () => {
        const res = await axios.get('https://onlineparttimejobs.in/api/rateCard/labourCharge', {
            headers: {
    ...
    const deletData = async (id) => {
        try {
            const res = await axios.delete(`https://onlineparttimejobs.in/api/rateCard/labourCharge/delete_labourCharge/${id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
    ...
    return (
        <>
            <div className="aiz-main-content">
                    ...
                    <div className="card">
                        {/* <div className="card-header">
                            ...
                            <div className="col-md-6 text-md-right">
                                <Link to="/admin/labourCharge-rate/add" className="btn btn-info">
                        ...
                        <div className="card-body">
                            ...
                            <section className="form-section">
                                <div className="row">
                                    ...
                                    <div className="col-lg-3">
                                        <select className="form-select" aria-label="Default select example">
                                            <option selected>Metal Price</option>
                                            <option value={1}>Active</option>
                                            <option value={1}>Inactive</option>
                                        </select>
                                    </div>
                ...
                <table className="table table-3" id="table-to-xlsx" style={{ display: 'flex' }}>
                    <thead>
                        <tr>
                            <th class="table-secondary" scope="col">Title</th>
                            <th class="table-secondary" scope="col">Date</th>
...
export default LabourChargeRateCardComp