...
function MultiLingualAddIndexGroup({ data, item, i, addNewAttributeData, onChangeHandler, setValue, onChangeHandlesr }) {
    const { data: valDAta } = useGetCategoriesQuery(token);
    const idShow = window.localStorage.getItem('adminId')
    return <div className="card">
        <div className="card-header">
            <h5 className="mb-0 h6">Index Information ({item?.lable})</h5>
        </div>
...
export default MultiLingualAddIndexGroup