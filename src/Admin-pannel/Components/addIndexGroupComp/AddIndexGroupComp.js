...
import { useAddCategaryMutation, useAddInddexGroupMutation, useGetLanguagesQuery } from '../all-products/allproductsApi/allProductsApi';
// import MultiLangAddCelebrityCategory from './MultiLangAddCelebrityCategory';
import MultiLingualAddIndexGroup from './MultiLingualAddIndexGroup';


function AddIndexGroupComp() {
    const token = window.localStorage.getItem('token')


    const toastSuccessMessage = () => {
        toast.success("Index Group added Successfully", {
            position: "top-center"
        })
    };
    ...
    const submitEditCategoryData = async (data) => {
        ...
        const url = `https://onlineparttimejobs.in/api/influenceIndexGroup/${params.id}`
        try {
            ...
            alert('IndexGroup Edited Successfully')
            setspcOr(false)
        } catch (error) {
            alert('IndexGroup not Edited')
            setspcOr(false)
        }
    }

    // const [addcategory, res] = useAddCategaryMutation()
    const [addIndexGroup, res] = useAddInddexGroupMutation()
    useEffect(() => {
        if (res.isSuccess) {
            toastSuccessMessage()
            setspcOr(false)
    ...
    const addNewAttributeData = async (e) => {
        ...
        if (params?.id) {
            ...
            addIndexGroup({ data: { list: clone }, token: token })
    ...
    const getDetailCat = async () => {
        ...
        try {
            const res = await axios.get(`https://onlineparttimejobs.in/api/influenceIndexGroup/admin/${params.id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
                    ...
                    <div className="row">
                        <div className="col-lg-8 mx-auto">
                            {spcOr && <div className="preloaderCount">
                                ...
                                <h6>please wait your Index uploading</h6>
                            </div>}
                                ...
                                <TabContext value={value}>
                                    ...
                                    {val && val.map((item, i) => {
                                        return <TabPanel value={i}>
                                            <div className="card">
                                                <MultiLingualAddIndexGroup setValue={setValue} data={val} item={item} i={i} addNewAttributeData={addNewAttributeData} onChangeHandler={onChangeHandler} onChangeHandlesr={onChangeHandlesr} />
                                            </div>
...
export default AddIndexGroupComp