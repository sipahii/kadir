...
function AddGemstoneComp() {
    ...
    useEffect(() => {
        if (data) {
            const maped = data.map((item) => {
                return { name: "", code: '', description: '', slug: '', meta_title: '', meta_keyword: '', meta_description: '', sort_no: '', shape_id: '', country_id: '', language_id: item._id, isActive: false, lable: item.name }
            })
            setVal(maped)
        }
    }, [data]);
    ...
    const toastErrorMessage1 = () => {
        toast.error("Gemstone Not Updated ", {
            position: "top-center"
        })
    };
    const toastSuccessMessage2 = () => {
        toast.success("Gemstone Added", {
            position: "top-center"
        })
    };
    const toastErrorMessage2 = () => {
        toast.error("Gemstone Not Added", {
            position: "top-center"
        })
    };

    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/gemStone/${params?.uid}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const sendData = async () => {
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/gemstone/update_gemstone/${params?.uid}`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/gemstone/add_gemStone`, finalSendingD, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
                ...
                <TabContext value={value}>
                    ...
                    {val && val.map((item, i) => {
                        return <TabPanel value={i}>
                            <div className="card">
                                <AddGemstoneMultiLang setValue={setValue} data={val} params={params} item={item} i={i} sendData={sendData} onChangeHandler={onChangeHandler} onChangeThumbnailImage={onChangeThumbnailImage} showImageD={showImageD} setShowImageD={setShowImageD} onChangeHandleExcel={onChangeHandleExcel} />
                            </div>
...
export default AddGemstoneComp
