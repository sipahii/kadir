...
import { useBrandActiveMutation, useDeleteBrandMutation, useDeletedivitionMutation, useGetBrandsQuery, useGetDivitionQuery } from "../../all-products/allproductsApi/allProductsApi"
import { ToastContainer, toast } from "react-toastify";

function ListDistricte() {
    const [inputVal, setInputVal] = useState({ search: '' });
    const token = window.localStorage.getItem('token')
    const { isLoading, data } = useGetDivitionQuery(token);
    ...
    const [deleteBrand, response] = useDeletedivitionMutation();
    ...
    const toastErrorMessage = () => {
        toast.error("Division Update Faild..", {
            position: "top-center"
        })
    };
                ...
                <div className="card">
                    <div className="card-header row gutters-5">
                        <div className="col text-center text-md-left">
                            <h5 className="mb-md-0 h6">List Division</h5>
                        </div>
                        <ToastContainer />
                                ...
                                <tbody>
                                    ...
                                    {blankArr && blankArr.map((item, i) => {
                                        return <tr key={item._id}>
                                            ...
                                            <td style={{ display: 'table-cell' }}>{item?.sortNo}</td>

                                            <td style={{ display: "table-cell" }}>
                                               {item?.keyword?.length && item?.keyword?.map((val)=>{
                                                return <span style={{margin:"0 7px"}}>{val},</span>
                                               })}
                                            </td>

                                            <td className="text-right footable-last-visible" style={{ display: 'table-cell' }}>
                                                <Link to={`/admin/division/edit/${item.uid}`} className="btn btn-soft-primary btn-icon btn-circle btn-sm" title="Edit">
