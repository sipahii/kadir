...
function PodEnteris(props) {
    ...
    const sendData = async () => {
        ...
        fromData.append('customer_id', props.data[0].user_id);
        fromData.append('order_id', props.data[0]._id);
        fromData.append('createdBy', setId);
        fromData.append('AwbNo', clone.AwbNo);
        fromData.append('orderStatusId', clone.orderStatusId);
        fromData.append('rcDate', clone.rcDate);
        fromData.append('rcTime', clone.rcTime);
        fromData.append('remarks', clone.remarks);
        fromData.append('rcName', clone.rcName);
        fromData.append('rcRelation', clone.rcRelation);
        fromData.append('rcPhone', clone.rcPhone);
        fromData.append('courier_reference_no', clone.courier_reference_no);
        fromData.append('mode', clone.mode);
        fromData.append('courierName', clone.courierName);
        fromData.append('image', file);

        try {
            const res = await axios.post(`https://onlineparttimejobs.in/api/delivery/add_Delivery`, fromData)
            setLoader(false)
            setError(false)
