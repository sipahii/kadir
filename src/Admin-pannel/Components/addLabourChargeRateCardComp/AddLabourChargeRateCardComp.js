...
function AddLabourChargeRateCardComp() {
    const [inputval, setInputVal] = useState({
        name: '', country_id: '', metalType_id: '', description: '', default: false, isActive: false
    })
    ...
    const sendData = async () => {
        if (params?.id) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/rateCard/labourCharge/update_labourCharge/${params?.id}`, inputval, {
                    headers: {
            ...
            try {
                const res = await axios.post(`https://onlineparttimejobs.in/api/rateCard/labourCharge/add_labourCharge`, inputval, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
    ...
    const getShapeSelectData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/metalType`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
    ...
    const getByIdData = async () => {
        const res = await axios.get(`https://onlineparttimejobs.in/api/rateCard/labourCharge/${params?.id}`, {
            headers: {
                "Content-type": "application/json; charset=UTF-8",
                Authorization: `Bearer ${token}`,
            },
                ...
                <div className="px-15px px-lg-25px">
                    <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: '3px' }}>
                            <div className="">
                                <h1 className="h3">{params.id ? 'Update' : 'Add'} Labour Charge</h1>
                            </div>
                        </div>
                    </div>
                    <div className="card">
                        <div className="card-header">
                            <h5>{params.id ? 'Update' : 'Add'} Labour Charge</h5>
                        </div>
                            ...
                            <section className="form-section">
                                <div className="row">
                                    ...
                                    <div className="col-lg-6 mt-4">
                                        <select className="form-control form-select" name="metalType_id" value={inputval?.metalType_id} aria-label="Default select example" onChange={onChangeHandler}>
...
export default AddLabourChargeRateCardComp