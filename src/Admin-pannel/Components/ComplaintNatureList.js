...
import { useDeleteAttributesMutation, useDeleteComplaintNatureMutation, useGetAttributesQuery, useGetTComplaintNatureQuery } from "./all-products/allproductsApi/allProductsApi";

function ComplaintNatureList() {

    const { isLoading, data } = useGetTComplaintNatureQuery();
    console.log('data---', data)
    const [deleteComplaintNature, response] = useDeleteComplaintNatureMutation();

    const deleteComplaintNatureData = (id) => {
        deleteComplaintNature(id)
    };
                                    ...
                                    {data && data.map((item, i) => {
                                        return <tr key={i}>
                                            ...
                                            <i className="las la-cog" />
                                                ...
                                                <button type="button" onClick={() => deleteComplaintNatureData(item._id)} className="btn btn-soft-danger btn-icon btn-circle btn-sm confirm-delete">
