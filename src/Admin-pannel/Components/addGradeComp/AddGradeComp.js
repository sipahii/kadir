...
import { useAddCelebrityGradeMutation, useGetLanguagesQuery } from '../all-products/allproductsApi/allProductsApi';
import AddgradeMultiLang from './AddgradeMultiLang';


function AddGradeComp() {
    ...
    const submitEditGradeData = async (data) => {
        const url = `https://onlineparttimejobs.in/api/influence_gradeMaster/${params.id}`
        try {
            ...
            alert('Edit Grade Successfully')
            setspcOr(false)
        } catch (error) {
            alert('Grade not Edit')
            setspcOr(false)
        }
    };

    const [addGrade, res] = useAddCelebrityGradeMutation();
    ...
    const addNewAttributeData = async (e) => {
        ...
        if (params?.id) {
            ...
            addGrade({ data: { list: clone }, token: token });
    ...
    const getDetailCat = async () => {
        try {
            const res = await axios.get(`https://onlineparttimejobs.in/api/influence_gradeMaster/admin/${params.id}`, {
                headers: {
                    "Content-type": "application/json; charset=UTF-8",
                    Authorization: `Bearer ${token}`,
                },
                                ...
                                <TabContext value={value}>
                                    ...
                                    {val && val.map((item, i) => {
                                        return <TabPanel value={i}>
                                            <div className="card">
                                                <AddgradeMultiLang setValue={setValue} data={val} item={item} i={i} addNewAttributeData={addNewAttributeData} onChangeHandler={onChangeHandler} onChangeHandlesr={onChangeHandlesr} />
                                            </div>
