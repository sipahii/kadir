...
import { useAddComplaintTypeMutation, useAddNewAttributeMutation, useGetTComplaintTypeByIdQuery, useUpdateComplaintTypeMutation } from "./all-products/allproductsApi/allProductsApi";
...
function EditComplaintType() {
    ...
    const { data: comlaintTypeDataById } = useGetTComplaintTypeByIdQuery(params.id)

    useEffect(() => {
        if (comlaintTypeDataById) {
            const clone = { ...comlaintTypeDataById }
            setInputval(clone)
            setStatusD(clone.isActive)
        }
    }, [comlaintTypeDataById])
    ...
    const updateComplaintTypeData = async (e) => {
        e.preventDefault();
        document.getElementById("create-course-form").reset();
        try {
            const resp = await axios.put(`https://onlineparttimejobs.in/api/complainttype/update_ComplaintType/${params.id}`, inputval)
            toastSuccessMessage()
        } catch (error) {
            toastErrorMessage()
        }
            ...
            <div className="col-md-5">
                <div className="card">
                    ...
                    <div className="card-body">
                        <form id="create-course-form" onSubmit={updateComplaintTypeData}>
