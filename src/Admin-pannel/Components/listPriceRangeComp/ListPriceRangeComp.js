...
import { useCatagaryActiveMutation, useDeleteCelebrityPriceRangeMutation, useGetCelebrityPricerangeQuery, } from "../all-products/allproductsApi/allProductsApi";
import { ToastContainer, toast } from "react-toastify";


function ListPriceRangeComp() {
    const [inputVal, setInputVal] = useState({ search: '' });
    const [blankArr, setBlankArr] = useState([]);

    const token = window.localStorage.getItem('token')
    // const { isLoading, data, refetch } = useGetIndexGroupQuery(token);
    const { isLoading, data, refetch } = useGetCelebrityPricerangeQuery(token);
    ...
    const [deleteCelebrityPriceRange, res] = useDeleteCelebrityPriceRangeMutation();

    function deleteCategoryData(id) {
        deleteCelebrityPriceRange({ id, token })
    };
    ...
    const toastErrorMessage = () => {
        toast.error("Price Range Update Faild..", {
            position: "top-center"
        })
    };
                ...
                <div className="px-15px px-lg-25px">
                    <div className="aiz-titlebar text-left mt-2 mb-3">
                        <div className="row align-items-center">
                            <div className="col-md-6">
                                <h1 className="h3">All Price Range</h1>
                            </div>
                            <div className="col-md-6 text-md-right">
                                <Link to="/admin/priceRange/create" className="btn btn-primary">
                                    <span>Add New Price Range</span>
                                </Link>
                            </div>
                    ...
                    <div className="card">
                        ...
                        <div className="card-header d-block d-md-flex">
                            <h5 className="mb-0 h6">Price Range</h5>
...
export default ListPriceRangeComp;