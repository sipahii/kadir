...
function AddCollectionManagementComp() {
    ...
    const addFile = async (clonedObj) => {
        ...
        if (params?.uid) {
            try {
                const res = await axios.put(`https://onlineparttimejobs.in/api/collection/update_collection/${params?.uid}`, { list: cloned }, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
            ...
            try {
                const res = await axios.post('https://onlineparttimejobs.in/api/collection/add_Collection', { list: cloned }, {
                    headers: {
                        "Content-type": "application/json; charset=UTF-8",
                        Authorization: `Bearer ${token}`,
                    },
