...
function RoboticMaster() {
  ...
  const getStyleData = async () => {
    ...
    for (let i = 0; i < reqData.data.length; i++) {
      getCategoryName.push({
        name: reqData.data[i]?.name,
        _id: reqData.data[i]?._id,
        uid: reqData.data[i]?.uid,
      });
    }
  ...
  const getByIdData = async () => {
    const res = await axios.get(
      `https://onlineparttimejobs.in/api/customerPermit/user/${param?.id}`,
      {
        headers: {
          "Content-type": "application/json; charset=UTF-8",
          Authorization: `Bearer ${token}`,
  ...
  const postData = async () => {
    const obj = {
      ...
      blogsCatPermit: finalBlogcategD,
    };
          ...
          <div className="aiz-titlebar text-left mt-2 mb-3">
            <div className="row align-items-center">
              <div className="col-md-6">
                <h1 className="h3">
                  {param?.id ? "Edit" : "Add"} Robotic SubCategories
                </h1>
              </div>
            </div>
          </div>
