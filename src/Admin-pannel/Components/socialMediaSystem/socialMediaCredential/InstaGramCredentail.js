...
import { useGetInstagramCredentailQuery, useUpdateInstagramCredentailMutation } from '../../all-products/allproductsApi/allProductsApi';

function InstaGramCredentail() {

    const [inputVal, setInputval] = useState({
        client_id: '', client_secret: ''
    });

    const { isLoading, data } = useGetInstagramCredentailQuery();
    console.log('instagramcredentailD---', data)
    ...
    const [updateInstaGramCredentialD, response] = useUpdateInstagramCredentailMutation();

    const submitUpdateInstagramCredential = (e) => {
        e.preventDefault();
        const abc = { ...inputVal }
        updateInstaGramCredentialD(abc)
        document.getElementById("create-course-form").reset();
    };

    const toastSuccessMessage = () => {
        toast.success("Instagram Credential Updated Successfully !", {
            position: "top-center"
        })
    };

    const toastErrorMessage = () => {
        toast.error("Instagram Credential not Updated !", {
            position: "top-center"
        })
    }
            ...
            <div className="col-lg-6">
                <div className="card">
                    <div className="card-header">
                        <h3 className="mb-0 h6">Instagram</h3>
                    </div>
                    <div className="card-body">
                        <form className="form-horizontal" id="create-course-form" onSubmit={submitUpdateInstagramCredential}>
...
export default InstaGramCredentail