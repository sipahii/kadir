...
import { useGetGoogleCredentailQuery, useUpdateGoogleCredentailMutation } from '../../all-products/allproductsApi/allProductsApi';

function GoogleCredentail() {

    const [inputVal, setInputval] = useState({
        client_id: '', client_secret: ''
    });

    const { isLoading, data } = useGetGoogleCredentailQuery();
    console.log('googlecredentailD---', data)
    ...
    const [updateGoogleCredentialD, response] = useUpdateGoogleCredentailMutation();

    const submitUpdateGoogleCredential = (e) => {
        e.preventDefault();
        const abc = { ...inputVal }
        updateGoogleCredentialD(abc)
        document.getElementById("create-course-form").reset();
    };

    const toastSuccessMessage = () => {
        toast.success("Google Credential Updated Successfully !", {
            position: "top-center"
        })
    };

    const toastErrorMessage = () => {
        toast.error("Google Credential not Updated !", {
            position: "top-center"
        })
    }
            ...
            <div className="col-lg-6">
                <div className="card">
                    <div className="card-header">
                        <h3 className="mb-0 h6">Google</h3>
                    </div>
                    <div className="card-body">
                        <form className="form-horizontal" id="create-course-form" onSubmit={submitUpdateGoogleCredential}>
...
export default GoogleCredentail